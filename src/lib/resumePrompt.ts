...
7. Every bullet must add information a recruiter could not already assume from the job title, and no two bullets may say the same thing in different words. Delete bullets like "Attended meetings", "Collaborated with the team", or a second bullet about the same system unless it carries new scope, ownership, or a result; prefer four distinct bullets over six overlapping ones. Name the thing: never write "improvements", "initiatives", "efforts", or "various X" — either state what changed and by how much, or cut the bullet. Never invent a number to fill that gap.
